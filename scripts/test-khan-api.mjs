...
async function test() {
  ...
  const cacheBuster = `${dateStr}-${randomId}-${timestamp}`;
  ...
  try {
    ...
    if (sessions) {
      ...
      sessions.slice(0, 5).forEach((s, i) => {
        ...
        if (s.correctCount !== undefined) {
          console.log(`     Correct: ${s.correctCount}/${s.problemCount}`);
        }
      });
    } else if (parsed.errors) {
      console.log("\n❌ GraphQL errors:", JSON.stringify(parsed.errors, null, 2));
    } else {
      console.log("\nResponse:", JSON.stringify(parsed, null, 2).slice(0, 500));
    }
  } catch {
    console.log("\nNot JSON:", body.slice(0, 500));
  }
}
