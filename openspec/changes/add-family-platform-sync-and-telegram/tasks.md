...
- [x] 1.1 Add Telegram recipient configuration for a single household parent receiver
...
- [ ] 2.3 Implement the IXL sync connector
- [ ] 2.4 Implement the Khan Academy sync connector
- [x] 2.5 Add fixed daily batch scheduling for the initial sync cadence
- [ ] 2.6 Stage Raz-Kids and Epic as follow-on connectors after the first rollout
- [ ] 2.7 Add sync job health tracking, retry state, and failure summaries
...
- [x] 6.4 Verify acceptance criteria for sync correctness, Telegram delivery behavior, and bridge idempotency before rollout
- [ ] 6.5 Pilot IXL and Khan Academy first before enabling later connectors
...
- [ ] B.1 Finalize the first-release connector runtime strategy for IXL and Khan Academy
- [ ] B.2 Implement the IXL sync connector on top of the normalized event pipeline
- [ ] B.3 Implement the Khan Academy sync connector on top of the normalized event pipeline
- [ ] B.4 Add fixed daily batch execution for the after-school sync window and any additional release-one sync windows
- [ ] B.5 Verify account-password plus managed-session handling for the supported connector flow
- [ ] B.6 Verify duplicate prevention, unmatched-event handling, and earliest-match-wins behavior against real imported platform evidence
...
- [ ] C.1 Finalize whether the pilot bridge runtime is the home computer, Synology NAS, or a split coordinator/sender model
- [ ] C.2 Implement the bridge worker that consumes `voice_push_tasks`
- [ ] C.3 Add retry, failure capture, and idempotent audio delivery behavior in the chosen runtime model
- [ ] C.4 Verify that bridge delivery failure never changes the successful homework submission result
- [ ] C.5 Run pilot validation for bridge observability, retry handling, and duplicate-send prevention
